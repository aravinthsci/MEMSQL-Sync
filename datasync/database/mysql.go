...
func (fetcher *MySQLFetcher) Fetch() error {
	...
	log.Print("command:" + listTableCmd)
	...
	for _, table := range tables {
		...
		go func(table string) {
			...
			if inserter.Host == "localhost" || inserter.Host == "127.0.0.1" {
				...
				log.Print("statement:::" + cleanTablesCmd)
	...
	for _, table := range tables {
		...
		go func(table string) {
			...
			if inserter.Host == "localhost" || inserter.Host == "127.0.0.1" {
				if inserter.IsContainer {
					hostOption := "-h" + inserter.Host
					cmd = exec.Command("memsql", "-u"+inserter.User, hostOption, "--enable-local-infile", "--execute="+query)
				} else {
					cmd = exec.Command("memsql", "-u"+inserter.User, "--enable-local-infile", "--execute="+query)
				}
				...
				cmd = exec.Command("memsql", "-u"+inserter.User, passwordOption, "-h"+inserter.Host, "--enable-local-infile", "--execute="+query)
			}
			var stderr bytes.Buffer
			cmd.Stderr = &stderr
			err := cmd.Run()
